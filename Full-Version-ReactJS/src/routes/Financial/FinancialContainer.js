      ...
      <Row>
        {_.map(data, (item) => (
          <Col lg={3} sm={6} key={uuidv4()}>
            <Panel className={classes.cashSummaryPanel} header={item.Name}>
              <p className={classes.cashSummaryValue}>$ {item.Value}</p>
              {item.DiffChanged ? (
                <div>
  ...
  <div>
    <div className={classes.boxHeader}>
      ...
      <Link to="/apps/clients">
        <Button bsSize="small" className={classes.buttonWithIcon}>
...
const renderDashboardSettings = (data) => (
  <div>
    ...
    <Panel>
      <ListGroup fill>
        {_.map(data, (setting) => (
          <ListGroupItem key={uuidv4()} className={classes.settingsListItem}>
  ...
  render() {
    return (
      <div className={classes.mainWrap}>
        <Row>
          <Col lg={12}>{renderSection(renderCashSummary, this.state.CashSummary)}</Col>
        </Row>
        <Row>
          <Col lg={5}>{renderSection(renderMoneyMapChart, moneyMapChartData)}</Col>
          <Col lg={7}>{renderSection(renderRecentFunding, this.state.RecentFunding)}</Col>
        </Row>
        <Row>
          <Col lg={12}>{renderSection(renderInvoices, this.state.Invoices)}</Col>
        </Row>
        <Row>
          <Col lg={9}>{renderSection(renderPerformanceChart, this.state.AccountPerformance)}</Col>
