...
const renderGallery = () => (
  <div>
    <p>
      <span className="text-white">Highstock</span> lets you create stock or general timeline charts
      in pure JavaScript.Including sophisticated navigation options like a small navigator series,
      preset date ranges, date picker, scrolling and panning.
    </p>
    ...
    <Row className="m-t-2">
      {renderGalleryItem(
        'Single Line Series',
        '/graphs/highstock/single-line-series',
        thumbSingleLine
      )}
      {renderGalleryItem(
        'Two panes, candlestick and volume',
        '/graphs/highstock/two-panes-candlestick-volume',
        thumbTwoPanesCandlestick
      )}
      {renderGalleryItem(
        'Compare Multiple Series',
        '/graphs/highstock/compare-multiple-series',
        thumbCompareMultipleSeries
      )}
      {renderGalleryItem(
        '52k points with data grouping',
        '/graphs/highstock/points-with-data-grouping',
        thumbPointsWithDataGrouping
      )}
      {renderGalleryItem('Intraday area', '/graphs/highstock/intraday-area', thumbIntradayArea)}
      {renderGalleryItem(
        'Intraday with breaks',
        '/graphs/highstock/intraday-with-breaks',
        thumbIntradayWithBreaks
      )}
      {renderGalleryItem(
        'Intraday candlestick',
        '/graphs/highstock/intraday-candlestick',
        thumbIntradayCandlestick
      )}
      {renderGalleryItem(
        'Flags marking events',
        '/graphs/highstock/flags-marking-events',
        thumbFlagsMarkingEvents
      )}
    </Row>

    <Divider className="m-t-2">Chart types</Divider>
    ...
    <Row className="m-t-2">
      ...
      {renderGalleryItem(
        'Point Markers Only',
        '/graphs/highstock/point-markers-only',
        thumbPointMarkersOnly
      )}
    </Row>
  </div>
)
...
class HighstockContainer extends RoutedComponent {
  ...
  render() {
    ...
    return (
      <Row className="m-t-3">
        <Col lg={8}>{Chart ? <Chart /> : renderGallery(this.props.routeParams.chartId)}</Col>
        <Col lg={4}>
          <TreeNavigator currentPath={this.props.location.pathname}>
            <TreeNavigator.Branch title="General" eventKey="general">
              <Link to="/graphs/highstock/general">General</Link>
              <Link to="/graphs/highstock/single-line-series">Single line series</Link>
              <Link to="/graphs/highstock/two-panes-candlestick-volume">
                Two panes, candlestick and volume
              </Link>
              <Link to="/graphs/highstock/compare-multiple-series">Compare Multiple Series</Link>
              <Link to="/graphs/highstock/points-with-data-grouping">
                52k points with data grouping
              </Link>
              <Link to="/graphs/highstock/intraday-area">Intraday area</Link>
              <Link to="/graphs/highstock/intraday-with-breaks">Intraday with breaks</Link>
              <Link to="/graphs/highstock/intraday-candlestick">Intraday candlestick</Link>
              <Link to="/graphs/highstock/flags-marking-events">Flags marking events</Link>
              {/*
            ...
            <TreeNavigator.Branch title="Chart Types" eventKey="chart-types">
              <Link to="/graphs/highstock/spline">Spline</Link>
              <Link to="/graphs/highstock/step-line">Step Line</Link>
              <Link to="/graphs/highstock/area">Area</Link>
              {/*
                            ...
                            */}
              <Link to="/graphs/highstock/candlestick">Candlestick</Link>
              <Link to="/graphs/highstock/ohlc">OHLC</Link>
              <Link to="/graphs/highstock/column">Column</Link>
              {/*
                            <Link to='/graphs/highstock/column-range'>
                                Column Range
                            </Link>
                            */}
              <Link to="/graphs/highstock/point-markers-only">Point Markers Only</Link>
            </TreeNavigator.Branch>
          </TreeNavigator>
        </Col>
      </Row>
