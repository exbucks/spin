...
const renderStockValue = (value, percentage, growth, additionalClass = '') => {
  ...
  return (
    <div className={valueClass}>
      ...
      <span className={classes.stockValuePrice}>{value}</span>
      {percentage !== null ? (
  ...
  <Panel>
    <Row>
      <Col md={2} sm={4} xs={6}>
        ...
        <p className={classes.favoriteScoreTitle}>Action Score</p>
        <p className={classes.favoriteScoreTitle}>Quality Score</p>
        <p className={classes.favoriteScoreTitle}>Value Score</p>
        <p className={classes.favoriteScoreTitle}>Growth Score</p>
      </Col>
      {_.map(favoriteStocks, (stock) => (
        <Col md={2} sm={4} xs={6} key={stock._id}>
          <h2 className={classes.favoriteTitle}>{stock.Name}</h2>
          {renderStockValue(stock.Value, stock.Percentage, stock.Growth, classes.favoriteValue)}
          <p className={classes.favoriteScore}>
            <Label outline bsStyle="custom" customColor={Colors.grayLighter}>
              A
            </Label>
            <small className="text-white">{stock.Scores.Action}</small>
          </p>
          <p className={classes.favoriteScore}>
            <Label outline bsStyle="custom" customColor={Colors.grayLight}>
              Q
            </Label>
            <small className="text-white">{stock.Scores.Quality}</small>
          </p>
          <p className={classes.favoriteScore}>
            <Label outline bsStyle="custom" customColor={Colors.gray}>
              V
            </Label>
            <small className="text-white">{stock.Scores.Value}</small>
          </p>
          <p className={classes.favoriteScore}>
            <Label outline bsStyle="custom" customColor={Colors.grayLighter}>
              G
            </Label>
            <small className="text-white">{stock.Scores.Growth}</small>
          </p>
        </Col>
      ))}
    </Row>
...
const renderStockStats = (stats) => (
  ...
  >
    <Table fill striped className={classes.statsTable}>
      <thead>
        <tr>
          <th></th>
          <th>2013</th>
...
const renderSelectedStocks = (name, stocks) => (
  <Panel header={name}>
    <Table fill striped className={classes.selectedStocksTable}>
      <thead>
        <tr>
          <th></th>
          <th>Price</th>
        ...
        {_.map(stocks, (stock) => (
          <tr key={stock._id}>
            <td className="text-white">
              {stock.Name}
              <small className="text-muted">{` (${stock.ShortName})`}</small>
            </td>
            <td>{renderStockValue(stock.ValuePercentage + '%', null, stock.Growth)}</td>
            <td className="text-right">{stock.Scores.Action}</td>
            <td className="text-right">{stock.Scores.Quality}</td>
            <td className="text-right">{stock.Scores.Value}</td>
            <td className="text-right">{stock.Scores.Growth}</td>
          </tr>
        ))}
      </tbody>
    </Table>
          ...
          <div className={classes.detailedStockHeader}>
            <div>
              ...
              {renderStockValue(
                ...
                details.Percentage,
                details.Growth,
                classes.detailedStockHeaderValue
              )}
            </div>
            <Label bsStyle="custom" customColor={Colors.grayLighter}>
              Score: {details.Score}
            </Label>
          </div>
        }
  ...
  render() {
    return (
      <div className={classes.mainWrap}>
        <Row>
          <Col lg={12}>{renderSection(renderFavoriteStocks, this.state.FavoriteStocks)}</Col>
        </Row>
        <Row>
          <Col lg={8}>{this.renderDetailedStock(this.state.DetailedStock)}</Col>
          <Col lg={4}>{renderSection(renderStockStats, this.state.Stats)}</Col>
        </Row>
        <Row>
          <Col lg={6}>{renderSelectedStocks('Cheap Stocks', this.state.CheapStocks)}</Col>
