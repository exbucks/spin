...
const renderSummary = (summaryData) => (
  <div>
    <Row className={classes.summary}>
      ...
      <Col md={3} sm={8} xs={7}>
        <Divider>Daily Change</Divider>
        <p className={classes.summaryLargeValue}>
          {summaryData.DailyChange.Value}
          <small> Euro </small>
          <small
            className={
              summaryData.DailyChange.Diff < 0 ? classes.summaryValueLess : classes.summaryValueMore
            }
          >
    ...
    <Row className={classes.overall}>
      <Col md={5} sm={6} xs={12} className="text-center">
        <span className={classes.overallEntry}>
          ...
          <span className={classes.overallValue}>
            {numeral(summaryData.MarketCap).format('0,0')} Euro
          </span>
        </span>
      </Col>
      <Col md={3} sm={6} xs={12} className={`text-center`}>
        <span className={classes.overallEntry}>
          ...
          <span className={classes.overallValue}>
            {numeral(summaryData.Total).format('0,0')} BTC
          </span>
        </span>
      </Col>
    </Row>
...
const renderExchangePanel = () => (
  <Panel className={classes.exchangePanel}>
    <Form inline>
      <FormGroup>
        <span className={`${classes.exchangePanelTitle} visible-lg-inline`}>Buy or Sell BTC</span>
        ...
        <InputGroup className={classes.exchangeInput}>
          ...
          <InputGroup.Addon>Amount BTC</InputGroup.Addon>
        </InputGroup>
        ...
        <InputGroup className={classes.exchangeInput}>
          ...
          <FormControl type="number" placeholder="0.00" />
          <InputGroup.Addon>Price USD</InputGroup.Addon>
        </InputGroup>
      </FormGroup>
      <div className={classes.exchangeActions}>
        <Button bsStyle="primary" className={classes.exchangeAction}>
          <i className="fa fa-fw fa-download"></i>Buy BTC
        </Button>
        <Button bsStyle="danger" className={classes.exchangeAction}>
...
const renderOrders = (orders) => {
  const renderTime = (time, increase) => (
    <td>
      <i
        className={`fa fa-fw ${
          increase ? classes.iconIncrease + ' fa-caret-up' : classes.iconDecrease + ' fa-caret-down'
        }`}
      ></i>
      {time}
    </td>
      ...
      <tbody>
        {_.map(data, (order) => (
          <tr key={uuidv4()}>
            ...
            <td>{numeral(order.Btc).format('0,0.0000')}</td>
            <td>{numeral(order.Usd).format('0,0')}</td>
          </tr>
        ))}
      </tbody>
    </Table>
  ...
  render() {
    return (
      <div className={classes.mainWrap}>
        <Row>
          <Col md={12}>{renderSection(renderSummary, this.state.pricingSummary)}</Col>
        </Row>
        ...
        <Row>
          <Col md={12}>{renderSection(renderExchangePanel)}</Col>
        </Row>

        <Row>
          <Col md={12}>{renderSection(renderOrders, this.state.Orders)}</Col>
        </Row>
