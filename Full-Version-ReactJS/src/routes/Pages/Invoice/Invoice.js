    ...
    return (
      <Row>
        <Col lg={12}>
          ...
          <Panel className={classes.panelWhite}>
            <Row>
              <Col md={4}>
                ...
                <p className="m-b-0">{Invoice.Sender.AddressLine1}</p>
                <p className="m-b-0">{Invoice.Sender.AddressLine2}</p>
              ...
              <Col md={4} mdOffset={4} className="text-right">
                ...
                <a href="javascript: void(0)">#{Invoice.Id}</a>
                <p className="m-b-0">
                  {'to: '}
                  <strong className="text-gray-darker">{Invoice.Receiver.Name}</strong>
                </p>

                <p className="m-b-0 m-t-2">{Invoice.Receiver.AddressLine1}</p>
                <p className="m-b-0">{Invoice.Receiver.AddressLine2}</p>
                <p className="m-b-0">
                  <abbr title="Phone">Phone:</abbr>
                ...
                {_.map(Invoice.Items, (invoice, index) => (
                  <tr key={invoice._id}>
                    ...
                    <td className="text-right">
                      {numeral(parseFloat(invoice.Price) * parseInt(invoice.Quantity)).format(
                        '$0,0.00'
                      )}
                    </td>
                  </tr>
            ...
            <Row>
              <Col mdOffset={6} md={6}>
                <dl className="dl-horizontal text-right">
                  <dt>Sub-Total</dt>
                  <dd>{numeral(priceSum).format('$0,0.00')}</dd>
                  <dt>VAT</dt>
                  <dd>{numeral(priceSum * 0.23).format('$0,0.00')}</dd>
          ...
          <div className="m-t-1">
            <Button bsStyle="primary">Go to Paymaent</Button> <Button>Print</Button>{' '}
            <DropdownButton id="dropdown-save-as" title="Save as">
              <Dropdown.Item eventKey="1">
                <i className="fa fa-fw text-gray-lighter fa-file-pdf-o m-r-1"></i> Save as .pdf
              </Dropdown.Item>
              <Dropdown.Item eventKey="2">
                <i className="fa fa-fw text-gray-lighter fa-file-excel-o m-r-1"></i> Save as .xls
                (Excel)
              </Dropdown.Item>
              <Dropdown.Item eventKey="3">
                <i className="fa fa-fw text-gray-lighter fa-file-text-o m-r-1"></i> Save as .txt
                (Text)
              </Dropdown.Item>
              <Dropdown.Item eventKey="4">
                <i className="fa fa-fw text-gray-lighter fa-file-word-o m-r-1"></i> Save as .doc
                (Word)
              </Dropdown.Item>
