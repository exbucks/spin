...
const renderChartWithLegend = (chartData) => (
  <Media>
    ...
    <Media.Body>
      {_.map(chartData, (v, i) => (
        <div key={i}>
          <i className="m-l-1 fa fa-fw fa-circle" style={{ color: v.color }}></i>
      ...
      <Row>
        {_.map(data, (item) => (
          <Col sm={6} xs={6} key={item._id}>
            ...
            <p className={classes.itemsGroupValue}>{item.value}</p>
          </Col>
        ))}
      </Row>
    </div>
...
const renderTasksPanel = (tasks) => {
      ...
      <Media.Body>
        <div className="checkbox">
          <label>
            ...
            <p className="m-t-1 small">{moment(task.Date).format('DD-MMM-YYYY, HH:mm')}</p>
          </label>
        </div>
      </Media.Body>
  ...
  return (
    ...
    >
      <ListGroup fill>
        {_.map(tasks, (task) => (
          <ListGroupItem className={classes.listItemTransparent} key={task._id}>
            {renderTask(task)}
          </ListGroupItem>
        ))}
      </ListGroup>
  ...
  const renderProject = (project) => (
    <div>
      <div className={classes.projectTitle}>
        <Link to="/apps/task-details">{project.Title}</Link>
        <Label outline bsStyle="custom" customColor={statusToColor(project.Status)}>
          {project.Status}
        </Label>
      </div>
      <div className={classes.slimBarWrap}>
        <ProgressBar now={project.Completness} className={classes.slimBar} />
      </div>
      <Row className="m-b-2">
    ...
    >
      <ListGroup fill>
        {_.map(projects, (project) => (
          <ListGroupItem className="no-bg" key={project._id}>
...
const renderTimeline = (timeline) => {
  ...
  const renderTimelineItem = (entry) => (
    ...
    >
      <Timeline.ItemHeader
        primaryText={
          <p>
            <Label outline bsStyle="custom" customColor={titleToColor(entry.Title)}>
              {entry.Title}
            </Label>
          </p>
        }
        secondaryText={
          <p className={`text-white ${entry.Disabled ? classes.taskTitleComplete : ''}`}>
            {entry.Text}
          </p>
        }
      />
  ...
  return (
    <Panel
      ...
      maxHeight={639}
    >
      <Timeline itemAlignment="vertical-inner-date">
  ...
  render() {
    return (
      <div className={classes.mainWrap}>
        <Row>
          <Col lg={12}>
            <p className={classes.headParagraph}>{faker.lorem.paragraph()}</p>

            <Row>
              <Col lg={3}>
                {renderSection(renderPayments, _.pick(this.state, 'Payments', 'Invoices'))}
              </Col>
              <Col lg={3}>{renderSection(renderAllTasks, this.state.AllTasks)}</Col>
              <Col lg={3}>{renderSection(renderAllProjects, this.state.AllProjects)}</Col>
              <Col lg={3}>{renderSection(renderMyStats, this.state.MyStats)}</Col>
            </Row>

            <Row className="m-t-3">
              <Col lg={4}>{renderSection(renderTasksPanel, this.state.Tasks)}</Col>
              <Col lg={4}>{renderSection(renderTimeline, this.state.Timeline)}</Col>
              <Col lg={4}>{renderSection(renderMyProjectsPanel, this.state.MyProjects)}</Col>
            </Row>
          </Col>
        </Row>
      </div>
