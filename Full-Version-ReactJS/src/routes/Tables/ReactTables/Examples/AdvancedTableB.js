  ...
  render() {
    return (
      <div className={classes.priceEditor}>
        ...
        <Button bsSize="sm" bsStyle="info" onClick={this.updateData}>
          Save
        </Button>
      </div>
    );
  ...
  render() {
    const regionCheckBoxes = regions.map((region) => (
      <span key={`span-${region}`} className={classes.regionCheck}>
        ...
        />
        <label key={`label-${region}`} htmlFor={region}>
          {region}
        </label>
      </span>
    ));
    return (
      <div ref="inputRef" className={classes.regionChecks}>
        {regionCheckBoxes}
        <button className="btn btn-info btn-xs textarea-save-btn m-l-1" onClick={this.updateData}>
          Save
        </button>
      </div>
    );
...
class AdvancedTableB extends React.Component {
  ...
  createCustomToolbar(props) {
    ...
    return (
      <div>
        <Col md={6}>{tableName && <h4 className="m-t-0">{tableName}</h4>}</Col>
        <Col md={6} className="text-right">
          <div className={classes.customToolBar}>
            <div className={classes.search}>
              <InputGroup bsSize="sm">
                {searchField}
                <InputGroup.Button>{clearBtn}</InputGroup.Button>
              </InputGroup>
            </div>
    ...
    return (
      ...
      >
        ...
        <TableHeaderColumn
          ...
          customEditor={{
            getElement: createSelectionEditor,
            customEditorParameters: { items: statuses }
          }}
          width="120px"
        >
          Status
        ...
        <TableHeaderColumn
          ...
          customEditor={{
            ...
            customEditorParameters: { currencies: currencies }
          }}
        >
          Earnings
        </TableHeaderColumn>
