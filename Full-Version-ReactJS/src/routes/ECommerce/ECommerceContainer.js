...
const renderSummary = (summary) => {
  const renderSummaryBox = (title, data, currency = '') => {
    ...
    return (
      ...
      >
        <p className={classes.summaryValue}>{`${currency} ${numeral(data.value).format('0,0')}`}</p>
        <span
  ...
  return (
    <Row>
      <Col md={4}>{renderSummaryBox('Total Revenue', summary.TotalRevenue, 'IDR')}</Col>
...
const renderLatestTransactions = (transactions) => {
  const renderTransactionRow = (transaction) => {
    const statusIconClasses = classNames('fa fa-circle', classes.transactionStatus, {
    ...
    return (
      <div className={classes.mainWrap}>
        ...
        <Row className={classes.sectionRow}>
          ...
          <Col md={6}>{renderSection(renderLatestTransactions, this.state.LatestTransactions)}</Col>
        </Row>
        <Row className={classes.sectionRow}>
          <Col md={6}>{renderSection(renderLatestComments, this.state.LatestComments)}</Col>
