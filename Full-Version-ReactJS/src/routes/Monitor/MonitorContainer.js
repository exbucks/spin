      ...
      <Media.Left>
        <div className={classes.memoryChartWrap}>
          <Charts.SparklineDonut
            data={[memoryData.Allocated, memoryData.InCache, memoryData.Available]}
            colors={[Colors.brandCerulean, Colors.brandCuriousBlue, Colors.brandEndaveour]}
            radius={56}
            innerRadius="30%"
          />
        </div>
    ...
    <Row>
      {_.map(
        ...
        (memoryEntry, index) => (
          <Col sm={4} key={index}>
            <small>
              <i className="fa fa-fw fa-circle" style={{ color: memoryEntry.color }}></i>{' '}
              {memoryEntry.name}
            </small>
  ...
  return (
    <Row>
      {_.map(volumesData, (volume, i) => (
        <Col key={volume._id} className={classes.volumeWidget} md={3} sm={6} xs={6}>
          ...
          <Row>
            <Col xs={6}>
              <div className="small">
                <i
                  className={`fa fa-fw fa-circle ${classes['dot' + volumeColors[i].colorName]}`}
                ></i>
                <br />
                Used Space
              </div>
      ...
      {_.map(drivesData, (drive) => (
        <tr key={drive._id}>
          ...
          <td>
            ...
            <span className="label-gray-lighter label label-outline">{drive.Mount}</span>
          </td>
          <td>
            <ProgressBar
    ...
    return (
      <Row>
        ...
        <Col lg={4}>
          <Row>
            <Col lg={12} sm={6}>
              <Panel header="System Monitoring" className={classes.monitorPanel}>
                <ListGroup fill>
              ...
              <Panel header="Network Monitoring" className={classes.monitorPanel}>
                <ListGroup fill>{sectionRender(renderNetworkPanel, this.state.Network)}</ListGroup>
              </Panel>
            </Col>

            <Col lg={12} sm={6}>
              <Panel header="Hardware Temperature" className={classes.monitorPanel}>
                <ListGroup fill>
                  {_.map(this.state.Temperatures, (tempData) => renderTemperaturePanel(tempData))}
                </ListGroup>
              </Panel>
            </Col>
          </Row>
