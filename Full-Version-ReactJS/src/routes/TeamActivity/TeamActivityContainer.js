...
const renderSummary = (data) => {
  ...
  const renderLineChart = (name, data, color, unit = '') => (
    <Col md={12} sm={6} xs={6}>
      <p className={classes.lineChartTitle}>
        <span className={classes.lineChartValue}>{`${data.Count}${unit}`}</span>
        {` ${name}`}
      </p>
  ...
  return (
    <Panel
      ...
      header={
        <div className={classes.summaryPanelHeader}>
          <p className={classes.summaryPanelHeaderTitle}>Last {data.Duration} Days</p>
    ...
    >
      <Row>
        <Col lg={12}>
          <p className={classes.chartTitle}>Happiness</p>
          <Media>
            <Media.Left>
              <Charts.SparklineDonut
                data={[data.Happiness.Great, data.Happiness.Good, data.Happiness.Bad]}
                colors={[Colors.brandPrimary, Colors.brandInfo, Colors.grayLight]}
                radius={56}
                innerRadius="30%"
              />
            </Media.Left>
        ...
        <Col lg={12} className={classes.channelsContainer}>
          <p className={`${classes.chartTitle} ${classes.channelsTitle} `}>Channels</p>
          <Media>
    ...
    _.map(activities, (activity) => {
      ...
      return (
        ...
        >
          ...
          />
          <Timeline.ItemBody>
            <Link to="/pages/timeline">{activity.Content.Message}</Link>
            <br />
  ...
  render() {
    return (
      <Row className={classes.mainWrap}>
        <ReactInterval timeout={500} enabled={true} callback={() => this.addChartData()} />
        <Col lg={9} md={8}>
