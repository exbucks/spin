...
const renderComponentStatus = (name, component, color, metric = '%') => (
  <Col lg={3} sm={6}>
    <Panel
      ...
      header={
        <Media className={classes.filledMedia} style={{ backgroundColor: color }}>
          <Media.Body>
...
const renderProcessDetails = (processData) => {
    ...
    return (
      <div key={hash(processName)}>
        <div className={classes.generalRow}>
          <h4>{processName}</h4>
          <Label className="label-gray-lighter label-outline">v {version}</Label>
        </div>
        <div className="p-y-1">
          <SlimProgressBar now={usagePercentage} />
        </div>
        <div className={classes.generalRow}>
  ...
  return (
    ...
    >
      <p>Faker</p>
      <Table responsive fill standard className={classes.processTable}>
        <thead>
          <tr>
        ...
        <tbody>
          {_.map(processData, (data) => (
            <tr key={data._id}>
              <td>{processGeneral(data.Name, data.Space.Used, data.Space.Total, data.Version)}</td>
              <td>{processComponent(data.ProcessCount.Data, data.ProcessCount.Color)}</td>
              <td>
                {processComponent(
                  data.MemoryUsage.Data,
                  data.MemoryUsage.Color,
                  data.MemoryUsage.Metric
                )}
              </td>
              <td>
                {processComponent(data.CpuUsage.Data, data.CpuUsage.Color, data.CpuUsage.Metric)}
              </td>
              <td>
                {processComponent(data.DiskUsage.Data, data.DiskUsage.Color, data.DiskUsage.Metric)}
              </td>
            </tr>
          ))}
        </tbody>
  ...
  render() {
    return (
      <div className={classes.mainWrap}>
        <ReactInterval timeout={500} enabled={true} callback={() => this.simulateProcessData()} />
        <Row>
          {renderComponentStatus('Memory', this.state.MemoryUsage, Colors.brandPrimary, '%')}
          {renderComponentStatus('CPU', this.state.CpuUsage, Colors.brandSuccess, '%')}
          {renderComponentStatus('Traffic', this.state.Traffic, Colors.brandWarning, 'KB')}
          {renderComponentStatus('Disk I/O', this.state.DiskUsage, Colors.brandDanger, 'KB')}
        </Row>
        <Row>
