    ...
    <Grid fluid>
      <Row>
        <Col xs={4} className="text-center">
          <p className={classes.value}>{numeral(stats.followers).format('0,0')}</p>
          <span>Followers</span>
        </Col>
        <Col xs={4} className="text-center">
          <p className={classes.value}>{numeral(stats.following).format('0,0')}</p>
          <span>Following</span>
        </Col>
        <Col xs={4} className="text-center">
          <p className={classes.value}>{numeral(stats.comments).format('0,0')}</p>
          <span>Comments</span>
        </Col>
      </Row>
    </Grid>
      ...
      {_.map(commentsList, (comment) => (
        <ListGroupItem className={classes.filledListGroupItem} key={comment.id}>
          <Media>
            ...
            <Media.Body>
              <div>
                <p className="text-white m-y-0">{comment.user.name}</p>
                <p className="text-muted small">
                  on {moment(comment.date).format('MMM DD, YYYY')}
                  {' on '}
                  <a href="javascript:;">{comment.category}</a>
                </p>
              </div>
              <div className="m-t-1">
