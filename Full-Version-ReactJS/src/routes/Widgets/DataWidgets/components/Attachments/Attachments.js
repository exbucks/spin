      ...
      {_.map(attachments, (attachment) => (
        <Media key={attachment.id}>
          <Media.Left align="middle">
            <AvatarIcon type="rounded" backgroundColor={attachment.icon.color}>
          ...
          <Media.Body>
            <div className="flex-space-between">
              <div>
                <p className="text-white m-y-0">{attachment.title}</p>
          ...
          <Media.Right align="middle">
            <Button bsSize="sm">{attachment.actionIcon}</Button>
          </Media.Right>
        </Media>
      ))}
    </ListGroup>
