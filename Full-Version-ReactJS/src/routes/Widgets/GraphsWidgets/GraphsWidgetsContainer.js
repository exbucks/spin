...
class GraphsWidgetsContainer extends RoutedComponent {
  ...
  render() {
    return (
      <div>
        <p>
          <span className="text-white">Lorem ipsum dolor sit amet</span>, consectetur adipisicing
          elit. Officia ipsum dolor adipisci ad libero sunt nostrum harum nam odit doloremque eum
          expedita dolore dolores consectetur, maxime, obcaecati, repellat quibusdam. Sapiente,
          vitae. Aliquid velit voluptate perspiciatis omnis repudiandae obcaecati laborum nihil,
          culpa, voluptatibus. Excepturi, eaque perferendis.
        </p>
        <Row>
          <Col lg={8}>
        ...
        <Row>
          <Col lg={4}>
            {this.isPanelOpen('monitor') && <Monitor onClose={() => this.closePanel('monitor')} />}
            {this.isPanelOpen('statistics') && (
              <Statistics onClose={() => this.closePanel('statistics')} />
            )}
            {this.isPanelOpen('path') && <Path onClose={() => this.closePanel('path')} />}
          </Col>
          <Col lg={4}>
            {this.isPanelOpen('stock') && <Stock onClose={() => this.closePanel('stock')} />}
            {this.isPanelOpen('memory') && <Memory onClose={() => this.closePanel('memory')} />}
          ...
          <Col lg={4}>
            ...
            {this.isPanelOpen('network') && <Network onClose={() => this.closePanel('network')} />}
            {this.isPanelOpen('infobox') && <Infobox onClose={() => this.closePanel('infobox')} />}
            {this.isPanelOpen('satisfaction-results') && (
              <SatisfactionResults onClose={() => this.closePanel('satisfaction-results')} />
            )}
            {this.isPanelOpen('stats') && <Stats onClose={() => this.closePanel('stats')} />}
            <CpuBox />
          </Col>
        </Row>
      </div>
