  ...
  const renderRow = (rowData) => (
    <tr key={rowData._id}>
      ...
      <td className="text-white">
        {!asLinks ? rowData.Name : <a href="javascript:void(0)">{rowData.Name}</a>}
      </td>
      <td>{((parseInt(rowData.Views) / totalViews) * 100).toFixed(1)}%</td>
      <td>{numeral(rowData.Views).format('0,0')}</td>
      <td>{numeral(uniqueViews(rowData)).format('0,0')}</td>
...
const renderDetailedViewsChart = (chartConfig) => (
  <div>
    <div className={classes.chartSettings}>
      <ButtonGroup>
        <Button bsSize="small">Last 7 Days</Button>
        <Button bsSize="small">Live & Recorded</Button>
        <Button bsSize="small">All Events</Button>
        <Button bsSize="small">All media</Button>
        <Button bsSize="small">All Locations</Button>
      </ButtonGroup>
...
const renderViewsSummary = (views) => {
  ...
  return (
    <Panel className={classes.boxPanel}>
      {viewEntry('Total Views', numeral(views.TotalViews).format('0,0'), true)}
...
const renderRowTitle = (title) => (
  <Row>
    <Col lg={12} className={classes.titleRow}>
      <h4 className={classes.titleRowHeader}>{title}</h4>
    ...
    return (
      <div>
        ...
        <Row className={`${classes.leadRow} m-b-2`}>
          <Col lg={3}>{renderSection(renderViewsSummary, this.state.ViewsStatistics)}</Col>
          <Col lg={9}>{renderSection(renderDetailedViewsChart, this.state.ViewChart)}</Col>
        </Row>
        {renderRowTitle('Devices')}
        <Row className="m-b-2">
          <Col lg={10}>{renderViewsTable(this.state.Devices)}</Col>
          <Col lg={2}>{renderViewsChart(this.state.Devices)}</Col>
        </Row>
        {renderRowTitle('Media Type')}
        <Row>
          <Col lg={10}>{renderViewsTable(this.state.MediaType, true)}</Col>
