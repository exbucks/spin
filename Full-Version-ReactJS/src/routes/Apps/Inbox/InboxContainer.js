...
const renderPanelHeader = (router) => (
  <div className="flex-space-between">
    <div>
      <SearchBox phText="Search Inbox..." buttonStyle="default" className={classes.searchBox} />
    ...
    <div className={`hidden-xs ${classes.headerActions}`}>
      ...
      <Button bsStyle="success" className="m-l-1" onClick={() => router.push('/apps/new-email')}>
...
const renderPanelFooter = () => (
  <div className="flex-space-between">
    <p className="m-b-0">Showing 1 to 15 of 147 entries</p>
    <Pagination
      bsSize="medium"
      items={10}
      activePage={1}
      ...
      {_.map(messages, (message) => (
        <tr key={message._id}>
          <td>
            {message.Unread ? (
              <OverlayTrigger
                overlay={<Tooltip id="new-message-tooltip">You got a now message</Tooltip>}
                placement="top"
              >
                <i
          ...
          <td>
            <div className="flex-space-between">
              ...
              {message.Starred ? (
                <OverlayTrigger
                  overlay={<Tooltip id="remove-from-favs-tooltip">Remove from Favorites</Tooltip>}
                  placement="top"
                >
                  <i
                ...
                <OverlayTrigger
                  overlay={<Tooltip id="add-to-favs-tooltip">Add to Favorites</Tooltip>}
                  placement="top"
                >
                  <i
            ...
            <Media>
              ...
              <Media.Body>
                <a href="javascript: void(0)">
                  <span className="text-white">{message.Sender.Name}</span>
                </a>
                <br />
                <Media.Heading componentClass="span">{message.Sender.Address}</Media.Heading>
              </Media.Body>
            </Media>
          </td>
          <td>
            <Link to="/apps/email-details">
              <span className={message.Unread ? 'text-white' : 'text-muted'}>
                {message.Subject}
              </span>
            </Link>
            <br />
            <span>{truncate(message.Content, 50)}</span>
          </td>
          ...
          <td className="text-right">
            {message.HasAttachment ? <i className="fa fa-fw fa-lg fa-paperclip"></i> : null}
          </td>
          <td className="text-right">
            ...
            <br />
            <span>{moment(message.Date).format('hh:mm a')}</span>
          </td>
        </tr>
      ))}
    </tbody>
      ...
      <Row>
        ...
        <Col lg={10}>
          <Panel
            ...
            header={renderPanelHeader(this.props.history)}
