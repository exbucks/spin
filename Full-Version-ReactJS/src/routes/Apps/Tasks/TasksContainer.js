...
const renderActionsDropdown = (small = false) => (
  <DropdownButton
    ...
    pullRight
    title={<i className="fa fa-fw fa-gear"></i>}
      ...
      <tbody>
        {_.map(tasks, (task) => (
          <tr key={task._id}>
            ...
            <td>
              <PriorityButton selectedPriority={task.Priority} shortPriority size="small" />
            </td>
            <td>
              <span className="text-muted m-r-1">#{task.Id}</span>
              <Link to="/apps/task-details">{task.Title}</Link>
              <br />
            ...
            <td className="text-right">{moment(task.Date).format('DD-MMM-YYYY')}</td>
            <td className="text-right">{renderActionsDropdown()}</td>
          </tr>
        ))}
      </tbody>
    </Table>
...
const renderTasksGrid = (tasks) => {
  const renderTaskTile = (task) => (
    <Panel key={task._id}>
      <PriorityButton selectedPriority={task.Priority} size="small" />
      ...
      <ListGroup fill>
        ...
        <ListGroupItem className="no-bg">
          <div className="flex-space-between">
            <span>Updated: {moment(task.Date).format('dddd, DD MMMM YYYY')}</span>
...
class TasksContainer extends RoutedComponent {
  ...
  render() {
    ...
    return (
      <Row>
        <Col lg={3}>
          <Divider className={classes.leftPanelDivider}>Search</Divider>
          <SearchBox className={classes.leftPanelSection} />

          <Divider className={classes.leftPanelDivider}>Favorites</Divider>
          <FavoriteApps
            className={classes.leftPanelSection}
            appSelected={(url) => this.props.history.push(url)}
          />

          <Divider className={classes.leftPanelDivider}>Projects</Divider>
          <ProjectsList
            className={classes.leftPanelSection}
          ...
          <Divider className={classes.leftPanelDivider}>People</Divider>
          <UsersList className={classes.leftPanelSection} items={this.state.data.People} />
        </Col>
        <Col lg={9}>
          <div className="flex-space-between">
            <h3 className="f-w-300 m-t-1">
              <Link to={`/apps/projects/${listStyle}`}>Projects</Link>
              <span className="text-muted m-x-1">/</span>
              Analytics Redesign
              <Badge className="hidden-sm m-l-2">125 Tasks</Badge>
            </h3>
            <div className="hidden-xs">
              ...
              <Button bsStyle="primary" className="m-l-1">
                <span className="hidden-md hidden-sm hidden-xs">Add New Project</span>
