...
import { Media, Divider, AvatarImage } from 'components'
...
const renderChatMessage = (message, leftAligned) => (
  <Media
    className={`${classes.chatMessage} ${leftAligned && classes.leftAligned} ${
      !!parseInt(message.Active) && classes.active
    }`}
  ...
  >
    ...
    <Media.Body>
      <div className={classes.chatMessageContent}>{message.Content}</div>
      {leftAligned ? (
        <div className={classes.chatMessageFooter}>
          <span className="text-white">{message.User.Name}</span>
          <small>{` at ${getFormattedDate(message.Date)} `}</small>
        </div>
      ) : (
        <div className={classes.chatMessageFooter}>
          <small>{`${getFormattedDate(message.Date)} `}</small>
          <span className="text-white">{message.User.Name}</span>
        </div>
      )}
    </Media.Body>
      ...
      {_.map(messagesGroupedByDateSorted, (keyval) => {
        ...
        return (
          <div key={`chat-messages-${groupName}`}>
            {groupName !== 'Other' && <Divider textPosition="center">{groupName}</Divider>}
            {_.map(messages, (message, index) => renderChatMessage(message, index % 2 === 0))}
          </div>
        )
      })}
    </div>
