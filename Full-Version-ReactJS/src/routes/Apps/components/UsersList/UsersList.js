      ...
      {_.map(items, (person, index) => (
        <LinkContainer to="/apps/users/grid" key={index}>
          <NavItem className="no-bg">
            <Media>
              <Media.Left>
                <AvatarImage src={person.Avatar} size="small" />
              </Media.Left>
              <Media.Body>
                <h5 className="m-y-0">{person.Name}</h5>
                <p className="small text-gray-lighter m-b-0">{person.Address}</p>
              </Media.Body>
              <Media.Right>
                <OverlayTrigger
                  placement="left"
                  overlay={<Tooltip id="tooltip-status">{person.Status}</Tooltip>}
                >
                  <i
