...
const renderComment = (comment, index) => (
  <Media key={comment._id || comment.id || index} className="m-t-2">
    ...
    <Media.Body>
      <Media.Heading componentClass="h5">
        {comment.Sender.Name}
        <small className="m-l-1">{comment.DateAdded}</small>
      </Media.Heading>
      <p>{comment.Content}</p>
      <div>
        <span className="text-white">{`${comment.Likes} `}</span>

        <OverlayTrigger placement="top" overlay={<Tooltip id="tooltip-vote-up">Vote Up</Tooltip>}>
        ...
        <OverlayTrigger
          placement="top"
          overlay={<Tooltip id="tooltip-vote-down">Vote Down</Tooltip>}
        >
          <a href="javascript:void(0)">
        ...
        <a href="javascript: void(0)" className="m-l-1">
          Reply
        </a>
      </div>
