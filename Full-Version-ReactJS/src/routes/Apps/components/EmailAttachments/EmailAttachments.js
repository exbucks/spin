...
const fileTypeToIcon = (fileType) => {
  ...
  return (
    <span className="fa-stack fa-lg">
      <i className="fa fa-square fa-stack-2x" style={{ color: filesMap[fileType].color }}></i>
      <i
        className={`fa ${filesMap[fileType].icon} fa-stack-1x
...
const EmailAttachments = (props) => {
  ...
  return (
    <div {...otherProps}>
      <Divider>
        Attachments
        <Badge className="m-l-1">{items.length}</Badge>
      </Divider>
      <div>
        {_.map(items, (attachment) => (
          <div
            className={`flex-space-between ${classes.attachment}`}
            key={attachment._id || hash(attachment)}
          >
            <Media>
              <Media.Left align="middle">{fileTypeToIcon(attachment.Type)}</Media.Left>
              <Media.Body>
                <Media.Heading componentClass="span" className="text-white">
                  {attachment.FileName}
                </Media.Heading>
                <p className="m-b-0">
                  by {attachment.Owner}
            ...
            <Media className="m-t-0">
              <Media.Body className="text-right">
                <span>{moment(attachment.DateAdded).format('DD-MMM-YYYY')}</span>
                <br />
                <span>{moment(attachment.DateAdded).format('hh:mm A')}</span>
              </Media.Body>
              <Media.Right align="middle">
                <OverlayTrigger
                  placement="top"
                  overlay={<Tooltip id="tooltip-download">{props.actionTooltipText}</Tooltip>}
                >
                  <Button>
