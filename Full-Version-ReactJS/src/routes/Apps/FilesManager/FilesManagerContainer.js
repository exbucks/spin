          ...
          <tr key={directory._id}>
            <td>
              <Media>
                ...
                <Media.Body>
                  <p className="text-white m-b-0">{directory.name}</p>
                  <span>{directory.size} MB</span>
                </Media.Body>
              </Media>
            </td>
            <td>
            ...
            <td>
              {_.map(directory.shareUsers, (shareUser) => (
                <AvatarImage key={shareUser._id} src={shareUser.avatar} className="m-r-1" />
              ))}
            </td>
            <td>
              ...
              <div className="m-b-2">
                {_.map(file.shareUsers, (shareUser) => (
                  <span key={shareUser._id}>
                    <AvatarImage key={shareUser._id} src={shareUser.avatar} size="small" />{' '}
                  </span>
                ))}
              </div>
    ...
    return (
      <Row>
        ...
        <Col lg={10}>
          ...
          <Row>
            <Col md={6} sm={5} xs={12}>
              <h3 className="m-y-0 f-w-300">
                ...
                <span>Files Manager List</span>
                <Badge className="m-l-1">94</Badge>
              </h3>
              {/*     Mobile Actions  */}
              <Row>
          ...
          <Row>
            <Col lg={12}>
              {this.props.routeParams.type === 'files-list' &&
                renderSection(renderDirectoriesList, this.state.data.Directories)}
              {this.props.routeParams.type === 'files-grid' &&
                renderSection(renderFilesGrid, this.state.data.Files)}
            </Col>
          </Row>
        </Col>
      </Row>
