...
const renderTaskDetails = (task) => (
  <ListGroup className={classes.taskDetails}>
    <ListGroupItem className="flex-space-between">
      <h5 className={classes.detailsKey}>Project</h5>
      <div className={classes.detailsValue}>
        <Link to="/apps/tasks">{task.Project.Name}</Link>
      </div>
    </ListGroupItem>
    <ListGroupItem className="flex-space-between">
      <h5 className={classes.detailsKey}>Assigned by</h5>
      <div className={classes.detailsValue}>
        <Link to="/apps/profile-details">{task.AssignedBy.Name}</Link>
      </div>
    </ListGroupItem>
    <ListGroupItem className="flex-space-between">
      <h5 className={classes.detailsKey}>Start Date</h5>
      <div className={classes.detailsValue}>
        {moment(task.StartDate).format('ddd, DD MMM YYYY')}
      </div>
    </ListGroupItem>
    <ListGroupItem className="flex-space-between">
      <h5 className={classes.detailsKey}>Due Date</h5>
      <div className={classes.detailsValue}>{moment(task.DueDate).format('ddd, DD MMM YYYY')}</div>
    </ListGroupItem>
    <ListGroupItem className="flex-space-between">
      <h5 className={classes.detailsKey}>Priority</h5>
      <div className={classes.detailsValue}>
        <PriorityButton size="xsmall" shortPriority selectedPriority={task.Priority} />
      </div>
    </ListGroupItem>
    <ListGroupItem className="flex-space-between">
      <h5 className={classes.detailsKey}>Progress</h5>
      <div className={classes.detailsValue}>{Math.round(task.Progress * 100)}%</div>
    </ListGroupItem>
    <ListGroupItem className="flex-space-between">
      <h5 className={classes.detailsKey}>Task ID</h5>
      <div className={classes.detailsValue}># {task.Id}</div>
    </ListGroupItem>
    <ListGroupItem className="flex-space-between">
      <h5 className={classes.detailsKey}>Date Assigned</h5>
      <div className={classes.detailsValue}>
...
const renderHeader = () => (
  <div className={`${classes.taskHeader} flex-space-between`}>
    <h3 className="m-y-0 f-w-300">
      <Link to="/apps/projects/grid">Projects</Link>
      <span className="text-muted m-x-1">/</span>
      <Link to="/apps/tasks/grid">Tasks</Link>
      <span className="text-muted m-x-1">/</span>
      Task Details
    </h3>
...
const renderComments = (comments) => (
  <div className="m-t-3">
    <Divider>
      Comments
      <Badge className="m-l-1">{comments.length}</Badge>
    </Divider>
  ...
  >
    <Media className="m-b-3">
      <Media.Left align="middle">
        <OverlayTrigger
          overlay={<Tooltip id="open-close-task">Open / Close Task</Tooltip>}
          placement="top"
        >
          <Form.Check />
        </OverlayTrigger>
      ...
      <Media.Body>
        <h3 className={classes.taskPanelTitle}>
          <span className="text-muted m-r-1">#{data.Task.Id}</span>
          {data.Task.Title}
        </h3>
    ...
    <p className="m-b-3">{data.Task.Text}</p>
  ...
  render() {
    return (
      <div className={classes.mainWrap}>
        <Row>
          <Col lg={3}>
            <Divider className={classes.leftPanelDivider}>Task Details</Divider>
            {renderSection(renderTaskDetails, this.state.data.Task)}

            <Divider className={classes.leftPanelDivider}>Assigned To</Divider>
            <UsersList items={this.state.data.People} />
          </Col>
          <Col lg={9}>
