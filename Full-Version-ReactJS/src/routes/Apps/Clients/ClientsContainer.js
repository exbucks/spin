      ...
      <tbody>
        {_.map(clients, (client) => (
          <tr key={client._id} className={client === selectedItem ? 'active' : null}>
            ...
            <td>
              <FavoriteStar favorited={!!parseInt(client.Favoritted)} />
            </td>
            <td>
              <Media>
                ...
                <Media.Body>
                  ...
                  <p className="m-y-0">{client.Position}</p>
                </Media.Body>
              </Media>
            </td>
      ...
      <tbody>
        {_.map(companies, (company) => (
          <tr key={company._id} className={company === selectedItem ? 'active' : null}>
            ...
            <td>
              <Media>
                <Media.Left align="middle">{renderCompanyIcon(company.Name)}</Media.Left>
                <Media.Body className={classes.mediaFix}>
                  <a
                    href="javascript:void(0)"
                    onClick={() => changeItem(company, ENTRY_TYPE_COMPANY)}
                  >
                    {company.Name}
                  </a>
                  <p className="m-y-0">{company.Location}</p>
                </Media.Body>
              </Media>
            </td>
            <td>
...
const renderCompanyDetails = (company) => (
  <Panel className={classes.companiesPanel}>
    <Media>
      <Media.Left>
        <div className="p-t-1">{renderCompanyIcon(company.Name, 'large')}</div>
      </Media.Left>
      <Media.Body>
        <div className={classes.companyPanelName}>
          <h4>{company.Name}</h4>
          <FavoriteStar favorited={!!parseInt(company.Favoritted)} className="m-l-1" />
        </div>
    ...
    <p className="m-y-0">{company.ShortDesc}</p>

    <Divider>Tags</Divider>
    <div>
      {_.map(company.Tags, (label, index) => (
        <Label outline bsStyle="default" className="m-r-1" key={index}>
          {label}
        </Label>
      ))}
    </div>
    ...
    <Tabs defaultActiveKey="tab-1" className="m-t-3" id="company-details-panel">
      <Tab eventKey="tab-1" title="Contact Details" className={classes.detailsTab}>
        <Divider className="m-t-1">Contact</Divider>
        <dl className="dl-horizontal">
          <dt>Phone</dt>
          <dd className="text-white">{company.Contact.Phone}</dd>
          <dt>Email</dt>
          <dd>
            <a href="javascript:void(0)">{company.Contact.Email}</a>
          </dd>
        </dl>

        <Divider className="m-t-1">Address</Divider>
        <dl className="dl-horizontal">
          <dt>Address</dt>
          <dd className="text-white">
            <a href="javascript:void(0)">{company.Address.Line1}</a>
          </dd>
          <dt></dt>
          <dd className="text-white">
            <a href="javascript:void(0)">{company.Address.Line2}</a>
          </dd>
        </dl>
      </Tab>
        ...
        <Nav className="p-t-1">
          {_.map(company.Employees, (employee, index) => (
            <NavItem key={employee._id}>
              <Media>
                <Media.Left>
                  <AvatarImage src={employee.Avatar} size="small" />
                </Media.Left>
                <Media.Body>
                  <h5 className="m-y-0">{employee.Name}</h5>
                  <p className="small text-gray-lighter m-b-0">{employee.Location}</p>
                </Media.Body>
                <Media.Right align="middle">
                  <OverlayTrigger
                    overlay={<Tooltip id={`employee-status-${index}`}>{employee.Status}</Tooltip>}
                    placement="left"
                  >
                    <i
            ...
            <div>
              <div className={classes.tabsContainer}>
                <Nav bsStyle="tabs">
                  ...
                  <NavItem eventKey="companies">Companies</NavItem>
                </Nav>
                <div className={classes.tabSettings}>
                  <OverlayTrigger
                    overlay={<Tooltip id="tooltip-edit-hint">Edit</Tooltip>}
                    placement="top"
                  >
                    <Button>
