...
const renderActionsDropdown = (isSmall = false, dropup = false) => (
  <DropdownButton
    ...
    pullRight
    dropup={dropup}
    title={<i className="fa fa-fw fa-gear m-r-1"></i>}
            ...
            <td>
              <Media>
                ...
                <Media.Body className={classes.mediaFix}>
                  <h4 className="m-t-0">
                    <Link to="/apps/tasks/list">{project.Title}</Link>
                  </h4>
                  <Media.Heading className={classes.projectsEdited}>
                    Last Edited by:
                    <span className="text-white">{` ${project.LastEdit.Author}`}</span>
                    <br />
                    <span>{moment(project.LastEdit.Date).format('dddd, DD MMMM YYYY')}</span>
                  </Media.Heading>
                </Media.Body>
              </Media>
            </td>
            <td>
              <Label outline bsStyle="custom" customColor={projectStatusToColor(project.Status)}>
                {project.Status}
              </Label>
            </td>
            <td>
    ...
    <Panel className={classes.projectsPanel} key={project._id}>
      ...
      <div className="m-y-1">
        ...
        <h4 className={classes.inlineHeader}>
          <Link to="/apps/tasks/grid">{project.Title}</Link>
        </h4>
      </div>
      ...
      <ListGroup fill>
        ...
        <ListGroupItem className="no-bg">
          <div className="flex-space-between">
            <span>Start: {moment(project.StartDate).format('dddd, DD MMMM YYYY')}</span>
