...
const renderUserNetworks = (networks) => (
  <div className="text-center">
    {_.map(networks, (network, index) => (
      <OverlayTrigger
        overlay={<Tooltip id={`tooltip-${network}`}>{network}</Tooltip>}
        placement="top"
        key={index}
      >
    ...
    return (
      <Row>
        ...
        <Col lg={10}>
          ...
          />
          <Divider className="m-t-3 m-b-2">Image Results</Divider>
          <Row>
              ...
              <Col lg={4} md={6} key={result._id}>
                <Panel>
                  ...
                  <div className={classes.searchResultUserHeader}>
                    <h4 className="m-r-1">{result.Name}</h4>
                    <FavoriteStar favorited={!!parseInt(result.Favorited)} />
                  </div>
