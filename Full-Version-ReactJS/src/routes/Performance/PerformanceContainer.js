...
const renderDetailedSummary = (data) => {
  const renderRow = (row) => (
    <tr key={row._id}>
      ...
      <td>
        <Charts.SparklineLine data={row.DataResponse} width={130} height={20} />
      </td>
      ...
      <td>
        <Charts.SparklineLine data={row.DataThrughtput} width={130} height={20} />
      </td>
        ...
        <Col lg={12}>
          ...
          <Row>
            <Col lg={3} className="text-right">
              ...
              <p className={classes.avgResponseTimeLabel}>Average Response Time</p>

              <hr />

              <p className={classes.responsesSummary}>
                <span>{numeral(avgRequestsCount).format('0,0')}</span> requests over{' '}
                <span>{avgRequestsCountDuration}</span> min
              </p>
              <p className={classes.responsesSummary}>
                <span>~{numeral(avgRequestsCountDay).format('0,0')}</span> requests over{' '}
                <span>24</span> h
              </p>
            </Col>
            <Col lg={9}>
          ...
          <Row>
            <Col lg={12}>{renderSection(renderDetailedSummary, this.state.PartsPerformance)}</Col>
          </Row>
        </Col>
      </Row>
    );
