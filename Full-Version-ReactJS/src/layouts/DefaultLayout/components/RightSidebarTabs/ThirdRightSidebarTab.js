    ...
    {_.toArray(
      _.mapObject(userGroups, (userGroup, groupKey) => (
        <div key={`user-group-${groupKey}`}>
          <h6 className={`${classes.sectionTitle} m-t-2`}>{`${groupKey} (${userGroup.length})`}</h6>
                ...
                <Media>
                  ...
                  {user.UnreadMessages ? (
                    <Media.Right align="middle">
                      <Badge bsStyle={user.UnreadMessages.Recent ? 'danger' : 'default'}>
      ...
      <Panel
        header={
          <div className={classes.flexSpaceBetween}>
            <span>
              <a href="javascript:void(0)" className="small" onClick={() => this.toggleChat(false)}>
                <i className="fa fa-fw fa-chevron-left"></i>
              </a>
            </span>
            {chat.Receiver}
            <a href="javascript: void(0)">
              <i className="fa fa-fw fa-circle" style={{ color: statusToColor(chat.Status) }}></i>
            </a>
          </div>
        }
        footer={
          ...
          index % 2 === 0 ? (
            <Media key={message._id}>
              ...
              <Media.Body>
                <div
                  className={`${
                    message.Recent ? 'bg-primary' : 'bg-gray-dark'
                  } text-white b-r-a-3 p-x-2 p-y-2`}
                >
                  {message.Content}
                </div>
                <h6 className="m-y-1">
                  {message.User.Name}
                  <small className="m-l-1">{moment(message.Time).format('HH:mm a')}</small>
                </h6>
              </Media.Body>
            </Media>
          ) : (
