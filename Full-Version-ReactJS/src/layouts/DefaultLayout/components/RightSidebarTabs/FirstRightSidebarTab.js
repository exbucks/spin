      ...
      <tbody>
        {_.map(stock, (stockItem) => (
          <tr key={stockItem._id}>
            ...
            <td className={classes.stockValue}>{num(stockItem.Value).format('0,0.00')}</td>
            <td className="text-right">
              <Label bsStyle={parseFloat(stockItem.Diff) < 0 ? 'danger' : 'success'}>
                {stockItem.Diff}%
              </Label>
            </td>
          </tr>
            ...
            <Media.Body>
              <LinkContainer to="/apps/email-details">
                <div className={classes.clickableElement}>
                  <h5 className="m-b-0">
                    {email.Name}
                    <small className="m-l-1">{moment(email.Time).format('HH:mm a')}</small>
                  </h5>
